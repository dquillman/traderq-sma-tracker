# patch-142c.ps1 — Fix yfinance MultiIndex columns in _to_ohlc; bump v1.4.2
param(
  [switch]$Run,
  [int]$Port = 8602
)
...
# 2) Replace _to_ohlc with robust MultiIndex-safe version (use compiled regex options correctly)
$find = 'def\s+_to_ohlc\s*\([^\)]*\)\s*:\s*(?:.|\r|\n)*?^\s*def\s+_sma\s*\('
$opts = [System.Text.RegularExpressions.RegexOptions]::Singleline `
      -bor [System.Text.RegularExpressions.RegexOptions]::Multiline
$rx = New-Object System.Text.RegularExpressions.Regex($find, $opts)
