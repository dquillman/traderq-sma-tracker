# patch-133.ps1  — TraderQ v1.3.3 full app rewrite (safe backup, optional run)
param(
  [switch]$Run,
  [int]$Port = 8575
)
...
# Ensure yfinance in requirements (for crypto fallback)
if (Test-Path $ReqPath) {
  $req = Get-Content -Raw -Encoding UTF8 $ReqPath
  if ($req -notmatch '(?im)^\s*yfinance\s*$') {
    Add-Content -Encoding UTF8 $ReqPath "`nyfinance"
    Write-Host "[OK] Added yfinance to requirements.txt"
  }
}

# Write updated app.py (v1.3.3)
@'
...
def plot_chart(df: pd.DataFrame, ticker: str, pretouch_pct: float = 0, theme="Dark"):
    ...
    fig.update_layout(title=f"{ticker} – SMA 20/200 with Pretouch & Crosses",
...
if show_screener and len(tickers) > 0:
    ...
    st.dataframe(screener_df[["Ticker","Trend","Dist to SMA200 (%)","Dist to SMA20 (%)"]],
...
Write-Host "[OK] Wrote app.py v1.3.3"
...
if ($Run) {
  ...
  if (Test-Path $ReqPath) { python -m pip install --no-cache-dir -r $ReqPath }
  else { python -m pip install --no-cache-dir streamlit pandas numpy pandas-datareader pycoingecko yfinance plotly }
